...
const registerValidators = require('../utils/validators').registerValidators;
...
router.post('/login', [
  check('email')
    .isEmail().withMessage('Not valid value of email')
    .normalizeEmail(),
  check('password', 'Not valid value of password')
  ...
  try {
    ...
    if (!errorValid.isEmpty()) {
      req.flash('loginError', errorValid.array()[0].msg);
